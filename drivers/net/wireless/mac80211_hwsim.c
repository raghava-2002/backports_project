...
#define LOG_FUNC printk(KERN_INFO "Rathan: %s function called\n", __func__)
				    ...
				    struct hwsim_new_radio_params *param)
{
	...
	//Rathan prints the name of the radio means the name of the interface (virtual but names as phy0, phy1 etc)
	printk(KERN_DEBUG "Rathan hwname: %s\n", param->hwname);

	// to know what is info 
	//printk(KERN_DEBUG "Rathan info: %p\n", info);
