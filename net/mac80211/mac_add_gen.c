...
//this function for AP intiated triggers  random mac address generation for all stations associated with the AP
...
void trigger_generate_mac_add_ap_all(struct ieee80211_local *local, u64 mac_seed) {
    ...
    char *data;
    int total_size = ETH_ALEN + 16 + sizeof(mac_seed);
    ...
    list_for_each_entry_rcu(sta, &local->sta_list, list) {
        ...
        if (!gen) {
            ...
            memcpy(data + ETH_ALEN + 16, &mac_seed, sizeof(mac_seed));
        ...
        if(ccmp_reset){
            ...
            rndpn64 = trigger_generate_pn(mac_seed);
...
//this function for ap intiated triggers random mac address generation for the station
void trigger_generate_mac_add_sta(struct sta_info *sta, u64 mac_seed) {
    ...
    if (sta && (key = rcu_dereference(sta->ptk[sta->ptk_idx]))) {
        ...
        memcpy(data + ETH_ALEN + key->conf.keylen, &mac_seed, sizeof(mac_seed));
...
u64 trigger_generate_pn(u64 mac_seed) { 
    ...
    // Calculate l and h for time sake i make it constant
...
u64 generate_pn(void) {
    u64 t_epoch = ktime_get_real_seconds();  
    u64 pn;
    u64 pn_h, pn_l;
    int l, h;
    int bitrate = 94000; // Example bitrate in kbps
