...
//new function for sta to simplify 
...
/* void generate_mac_add_sta(struct ieee80211_local *local, long long int current_tp) {
    ...
    list_for_each_entry_rcu(sta, &local->sta_list, list) {
        const u8 *interface_mac_addr = sdata->vif.addr;
        unsigned char r_mac[ETH_ALEN];
        printk(KERN_DEBUG "Processing station base mac: %pM", interface_mac_addr);
        ...
        if (key) {
            ...
            printk(KERN_DEBUG "No PTK for station: %pM", interface_mac_addr);
            gen = false;
        }

        if (!gen) {
            memcpy(r_mac, interface_mac_addr, ETH_ALEN); // Use SA as the base MAC address
            printk(KERN_DEBUG "Using base MAC for random mac : %pM", interface_mac_addr);
        } else {
            memcpy(data + 16, interface_mac_addr, ETH_ALEN); // Use SA as the base MAC address
            //printk(KERN_DEBUG "Using base MAC: %pM", interface_mac_addr);
