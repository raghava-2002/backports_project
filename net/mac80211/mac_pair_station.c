...
void s_delete_entry(const unsigned char *s_base_mac) {
    unsigned int index = s_hash_function(s_base_mac);
    ...
    while (entry != NULL) {
        if (memcmp(entry->s_base_mac, s_base_mac, ETH_ALEN) == 0) {
            ...
            //printk(KERN_DEBUG "Mac pair: Entry with base MAC address %pM deleted.\n", s_base_mac);
    ...
    printk(KERN_DEBUG "Mac pair: Entry with random MAC address not found, nothing to delete entry %pM.\n", s_base_mac);
}

//if you find any issues regarding delete entry function, please check the deletion function from the MAT table make use of it 